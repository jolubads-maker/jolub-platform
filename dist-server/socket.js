    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send_message', async (data, callback) => {
            ...
            if (user && user.id !== Number(userId)) {
                ...
                if (callback)
                    callback({ status: 'error', error: 'Identity mismatch' });
                return;
            }
            ...
            try {
                ...
                if (!chat) {
                    ...
                    if (callback)
                        callback({ status: 'error', error: 'Chat creation failed' });
                    return;
                }
                // CHECK BLOCKING
                if (chat.isBlocked) {
                    ...
                    if (callback)
                        callback({ status: 'error', error: 'Chat is blocked' });
                    return;
                }
                ...
                if (callback)
                    callback({ status: 'error', error: error.message || 'Server error' });
            }
        });
        // MARCAR LEÍDO
